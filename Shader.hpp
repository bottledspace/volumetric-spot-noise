...
public:
    ...
    void uniform(const char *name, const glm::mat3 &mat) const {
		auto location = glGetUniformLocation(m_prog, name);
    ...
    void uniform(const char *name, const glm::vec3 &vec) const
        { glUniform3fv(glGetUniformLocation(m_prog, name),
            1, glm::value_ptr(vec)); }
    void uniform(const char *name, const glm::vec4 &vec) const
        { glUniform4fv(glGetUniformLocation(m_prog, name),
            1, glm::value_ptr(vec)); }
    template <size_t N>
    ...
    void uniform(const char *name, const Texture &texture, GLenum slot = GL_TEXTURE0) const {
		texture.bind(slot);
        glUniform1i(glGetUniformLocation(m_prog, name), slot-GL_TEXTURE0);
    }
    GLuint m_prog;
};